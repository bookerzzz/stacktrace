...
func display() {
	OutputStackTrace(os.Stdout, "bookerzzz")
}

func ExampleOutputStackTrace() {
	boo()
	// Output: though it depends on environment
	...
	// 00: [Function]OutputStackTrace [File]./stacktrace/stacktrace.go:62}
}

func ExampleGetStackTrace() {
	info := GetStackTrace("bookerzzz")
